...
#15 - Obter a matrícula e o nome de todos os alunos do 1º ano de ENGENHARIA MECÂNICA 
#que tiveram no 1º bimestre avaliação em  MATEMÁTICA I mas não em FÍSICA I.
conn = db_connect()
cursor = conn.cursor()
sql = """
SELECT a.alu_mat,
...
WHERE c.cur_nome = 'Engenharia Mecanica'
        ...
        AND a.alu_serie = 1
        AND a.alu_mat NOT IN (SELECT a.alu_mat
                                FROM aluno a
                                JOIN avaliacao av ON (a.alu_mat = av.alu_mat)
                                JOIN disciplina d ON (av.dis_cod = d.dis_cod)
                                JOIN curso c ON (a.cur_cod = c.cur_cod)
